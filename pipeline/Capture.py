    ...
    def get_frame(self, config_store: ConfigStore) -> Tuple[bool, cv2.Mat]:
        ...
        if self._video == None:
            ...
            try:
                ...
                pass
            self._video = cv2.VideoCapture(camera_id)
            self._video.set(
        ...
        if self._video == None:
            ...
            if config_store.remote_config.camera_id == "" or len(camera_id_split) != 3:
                ...
                for index, device in enumerate(devices):
                    if (
                        device.uniqueID()
                        == config_store.remote_config.camera_id.replace(":", "")
                    ):
...
class PylonCapture(Capture):
    ...
    def get_frame(self, config_store: ConfigStore) -> Tuple[bool, cv2.Mat]:
        if self._camera != None and self._config_changed(
            self._last_config, config_store
        ):
        ...
        if self._camera is None:
            if self._device == None:
                device_infos: list[pylon.DeviceInfo] = (
                ...
                for device_info in device_infos:
                    if (
                        ...
                        == config_store.remote_config.camera_id
                    ):
                        self._device = pylon.TlFactory.GetInstance().CreateDevice(
                            device_info
                        )
            if self._device == None:
                ...
                self._camera.GetNodeMap().GetNode(
                    "DeviceLinkThroughputLimitMode"
                ).SetValue("On")
                max_bandwidth = int(150e6) if self._mode == "color" else int(250e6)
                self._camera.GetNodeMap().GetNode("DeviceLinkThroughputLimit").SetValue(
                    max_bandwidth
                )
                self._camera.GetNodeMap().GetNode("ExposureAuto").SetValue("Off")
                self._camera.GetNodeMap().GetNode("AcquisitionMode").SetValue(
                    "Continuous"
                )
                self._camera.GetNodeMap().GetNode("ExposureTime").SetValue(
                    config_store.remote_config.camera_exposure
                )
                self._camera.GetNodeMap().GetNode("GainAuto").SetValue("Off")
                self._camera.GetNodeMap().GetNode("Gain").SetValue(
                    config_store.remote_config.camera_gain
                )
                ...
                if self._mode == "color":
                    ...
                    self._converter.OutputBitAlignment = (
                        pylon.OutputBitAlignment_MsbAligned
                    )

                    # Disable auto white balance
                    self._camera.GetNodeMap().GetNode("BalanceWhiteAuto").SetValue(
                        "Off"
                    )

                    self._camera.BalanceRatioSelector.SetValue("Red")
                    self._camera.BalanceRatio.SetValue(
                        config_store.remote_config.camera_balance_red
                    )

                    self._camera.BalanceRatioSelector.SetValue("Blue")
                    self._camera.BalanceRatio.SetValue(
                        config_store.remote_config.camera_balance_blue
                    )
        ...
        if self._camera is None:
            ...
            try:
                with self._camera.RetrieveResult(
                    50, pylon.TimeoutHandling_ThrowException
                ) as grab_result:
                ...
                if self._last_failed_time == None:
                    ...
                    time.time() - self._last_failed_time
                    > PylonCapture.failed_time_restart_timeout
                ):
