...
class FileConfigSource(ConfigSource):
    ...
    def update(self, config_store: ConfigStore) -> None:
        ...
        with open(self._config_filename, "r") as config_file:
            ...
            config_store.local_config.apriltags_stream_port = config_data[
                "apriltags_stream_port"
            ]
            config_store.local_config.objdetect_stream_port = config_data[
            ...
            config_store.local_config.obj_detect_max_fps = config_data[
                "obj_detect_max_fps"
            ]
        ...
        calibration_store = cv2.FileStorage(
            self._calibration_filename, cv2.FILE_STORAGE_READ
        )
    ...
    def update(self, config_store: ConfigStore) -> None:
        ...
        if not self._init_complete:
            ...
            self._camera_denoise_sub = nt_table.getDoubleTopic(
                "camera_denoise"
            ).subscribe(RemoteConfig.camera_denoise)
            self._camera_balance_red_sub = nt_table.getDoubleTopic(
                "camera_balance_red"
            ).subscribe(RemoteConfig.camera_balance_red)
            self._camera_balance_blue_sub = nt_table.getDoubleTopic(
                "camera_balance_blue"
            ).subscribe(RemoteConfig.camera_balance_blue)
            ...
            self._throttle_fps_sub = nt_table.getDoubleTopic("throttle_fps").subscribe(
                -1
            )
        ...
        config_store.remote_config.camera_balance_red = (
            self._camera_balance_red_sub.get()
        )
        config_store.remote_config.camera_balance_blue = (
            self._camera_balance_blue_sub.get()
        )
